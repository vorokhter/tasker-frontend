export function ArrowIcon({ onClick, direction }) {
  const rotate = direction ? "rotate(-90deg)" : "rotate(90deg)";
  return (
    <svg
      x="0px"
      y="0px"
      viewBox="0 0 17 24"
      width="24px"
      height="24px"
      fill="#5a6683"
      style={{ transform: rotate }}
      onClick={onClick}
    >
      <path d="M8.417,17.279c0,0,0.444,0.518,0.909,0.053c0.639-0.64,6.547-6.742,6.547-6.742    s0.818-0.813-0.266-0.813c-1.086,0-2.811,0-2.811,0s0-0.471,0-1.19c0-2.221,0-6.543,0-8.199c0,0,0.012-0.388-0.488-0.388    C11.806,0,6.1,0,5.397,0c-0.702,0-0.58,0.464-0.58,0.464c0,1.7,0,5.88,0,8.181c0,0.803,0,1.326,0,1.326s-2.155,0-3.045,0    c-0.891,0-0.071,0.776-0.071,0.776L8.417,17.279z" />
    </svg>
  );
}
